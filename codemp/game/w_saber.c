    ...
    gentity_t* other)
{
    ...
    float selfAngleLength = sqrt(pow(self->client->ps.viewangles[0], 2.0f) +
        pow(self->client->ps.viewangles[1], 2.0f) +
        pow(self->client->ps.viewangles[2], 2.0f));

    float otherAngleLength = sqrt(pow(other->client->ps.viewangles[0], 2.0f) +
        pow(other->client->ps.viewangles[1], 2.0f) +
        pow(other->client->ps.viewangles[2], 2.0f));
    ...
    gentity_t* other)
{
    ...
    float crossProductLength = sqrt(pow(crossProduct[0], 2.0f) +
        pow(crossProduct[1], 2.0f) +
        pow(crossProduct[2], 2.0f));
    ...
    return asin(sinAngle) * 180.0f / MATH_PI;
}

float JKU_radToDeg(float radians)
{
    return acos(radians) * 180.0 / MATH_PI;
}
...
void JKU_calculateSaberTrace(gentity_t* self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate, vec3_t saberTrMins, vec3_t saberTrMaxs, vec3_t lastValidStart, vec3_t lastValidEnd, trace_t* tr)
{
...
static QINLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate)
{
	...
	{
		if (hitEntity->client && (hitEntity->client->ps.userInt1) && WP_SaberCanArcBlock(hitEntity, self, tr.endpos, 0, MOD_SABER, qfalse)) 
		{
